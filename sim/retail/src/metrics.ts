...
import { TransactionResult } from './payment-sim';
...
export class MetricsAnalyzer {
   ...
   */
  async checkSupplyInvariant(): Promise<{ expected_supply: number; actual_supply: number }> {
    try {
   ...
   * Compute all metrics
   */
  async computeMetrics(): Promise<SimulationMetrics> {
    console.log('\nComputing metrics...');
  ...
  displayMetrics(): void {
    ...
    console.log('\n' + chalk.bold.blue('=== SIMULATION METRICS ===\n'));
    ...
    summaryTable.push(
      ['Total Transactions', this.metrics.total_transactions.toString()],
      [chalk.green('Successful'), this.metrics.successful.toString()],
      [chalk.red('Failed'), this.metrics.failed.toString()],
      ['Success Rate', `${this.metrics.success_rate.toFixed(2)}%`]
    );
    ...
    const latencyTable = new Table({
      head: [chalk.cyan('Latency Metric'), chalk.cyan('Value (ms)')],
      colWidths: [30, 20],
    });

    latencyTable.push(
      ['Average', this.metrics.latency.avg_ms.toString()],
      ['Minimum', this.metrics.latency.min_ms.toString()],
      ['Maximum', this.metrics.latency.max_ms.toString()],
      ['P50 (Median)', this.metrics.latency.p50_ms.toString()],
      ['P90', this.metrics.latency.p90_ms.toString()],
      ['P95', this.metrics.latency.p95_ms.toString()],
      ['P99', this.metrics.latency.p99_ms.toString()]
    );
    ...
    const throughputTable = new Table({
      head: [chalk.cyan('Throughput Metric'), chalk.cyan('Value')],
      colWidths: [30, 20],
    });

    throughputTable.push(
      ['Duration (s)', this.metrics.throughput.total_duration_s.toString()],
      ['TPS', this.metrics.throughput.txs_per_second.toString()]
    );
    ...
    const supplyTable = new Table({
      head: [chalk.cyan('Supply Check'), chalk.cyan('Value')],
      colWidths: [30, 20],
    });

    const matchColor = this.metrics.supply_check.matches ? chalk.green : chalk.red;
    supplyTable.push(
      ['Expected Supply', this.metrics.supply_check.expected_supply.toString()],
      ['Actual Supply', this.metrics.supply_check.actual_supply.toString()],
      ['Matches', matchColor(this.metrics.supply_check.matches ? '✓ YES' : '✗ NO')]
    );
